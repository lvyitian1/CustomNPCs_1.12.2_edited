...
public class ModelNpcDragon extends ModelBase {
   ...
   public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
      ...
      for(int i = 0; i < 5; ++i) {
         ...
         this.neck.rotateAngleY = this.func_40307_a(ad3[0] - ad[0]) * 3.1415927F / 180.0F * f11;
         this.neck.rotateAngleX = f14 + (float)(ad3[1] - ad[1]) * 3.1415927F / 180.0F * f11 * 5.0F;
         this.neck.rotateAngleZ = -this.func_40307_a(ad3[0] - (double)f13) * 3.1415927F / 180.0F * f11;
      ...
      this.head.rotateAngleY = this.func_40307_a(ad1[0] - ad[0]) * 3.1415927F / 180.0F * 1.0F;
      this.head.rotateAngleZ = -this.func_40307_a(ad1[0] - (double)f13) * 3.1415927F / 180.0F * 1.0F;
      this.head.render(f5);
      GlStateManager.pushMatrix();
      ...
      if (entitydragon.onGround) {
         for(int j = 0; j < 2; ++j) {
            ...
            this.frontLeg.rotateAngleX = MathHelper.cos((float)((double)(f * 0.6662F) + (j == 0 ? 0.0D : 3.141592653589793D))) * 0.6F * f1 + 0.45F + f7 * 0.5F;
            this.frontLegTip.rotateAngleX = -1.3F - f7 * 1.2F;
            this.frontFoot.rotateAngleX = 0.85F + f7 * 0.5F;
            this.frontLeg.render(f5);
            this.rearLeg.rotateAngleX = MathHelper.cos((float)((double)(f * 0.6662F) + (j == 0 ? 3.141592653589793D : 0.0D))) * 0.6F * f1 + 0.75F + f7 * 0.5F;
         ...
         for(int j = 0; j < 2; ++j) {
            ...
            this.wing.rotateAngleX = 0.125F - (float)Math.cos((double)f16) * 0.2F;
            this.wing.rotateAngleY = 0.25F;
            this.wing.rotateAngleZ = (float)(Math.sin((double)f16) + 0.125D) * 0.8F;
            this.wingTip.rotateAngleZ = -((float)(Math.sin((double)(f16 + 2.0F)) + 0.5D)) * 0.75F;
      ...
      for(int k = 0; k < 12; ++k) {
         ...
         this.neck.rotateAngleY = (this.func_40307_a(ad2[0] - ad[0]) * f11 + 180.0F) * 3.1415927F / 180.0F;
         this.neck.rotateAngleX = f14 + (float)(ad2[1] - ad[1]) * 3.1415927F / 180.0F * f11 * 5.0F;
         this.neck.rotateAngleZ = this.func_40307_a(ad2[0] - (double)f13) * 3.1415927F / 180.0F * f11;
         this.neck.rotationPointY = f8;
         this.neck.rotationPointZ = f9;
         this.neck.rotationPointX = f10;
         f8 = (float)((double)f8 + Math.sin((double)this.neck.rotateAngleX) * 10.0D);
