   ...
   public List getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, BlockPos pos) {
      if (args.length == 1) {
         ...
         if (command == null) {
            ...
            return (List)(args.length == 2 && command.runSubCommands() ? new ArrayList(command.subcommands.keySet()) : command.getTabCompletions(server, sender, (String[])Arrays.copyOfRange(args, 1, args.length), pos));
         }
      }
   }
